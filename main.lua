...
for i = 1, #fileList do
  ...
  --load the model and model z and clear them from gpu in every loop to save some space for computation in the gpu
  --if the gpu memory <= 2GB
  ...
  if fileList[i].scale == nil then
    ...
    print(fileList[i]," need to detect face")
    
    detectedFaces = faceDetector:detect(img)
    
    print("detect faces result = ", detectedFaces)
